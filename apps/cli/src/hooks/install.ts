...
export function registerInstallCommand(hooks: Command): void {
  hooks
    ...
    .action(async (options: { repoRoot: string; settingsPath: string; scope: string }) => {
      ...
      process.stdout.write(`Installed Claude hooks into ${settingsPath}\n`);
    });
}
...
export async function installClaudeHooks(input: {
  ...
  const agentMetricsHooks = buildClaudeHooksConfig({
    repoRoot: resolve(input.repoRoot)
  });

  const nextSettings = {
    ...existingSettings,
    hooks: mergeHookTrees(existingSettings.hooks, agentMetricsHooks)
  };
...
async function readSettingsJson(settingsPath: string): Promise<JsonObject> {
  try {
